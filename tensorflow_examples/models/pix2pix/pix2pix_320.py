...
## Changed the upsampling by adding a Upsample2D and reduced stride to 1
# def upsample(filters, size=3, norm_type='batchnorm', apply_dropout=False):
#     """Upsamples an input using UpSampling + Conv2DTranspose.

#     Args:
#         filters: number of filters
#         size: filter size (default kept at 3)
#         norm_type: Normalization type; either 'batchnorm' or 'instancenorm'.
#         apply_dropout: If True, adds the dropout layer.
...
#     # Step 1: Smoothly double the resolution
#     result.add(tf.keras.layers.UpSampling2D(size=2))  # Avoids checkerboard artifacts

#     # Step 2: Apply Transposed Convolution (now only refining features, not upsampling)
#     result.add(
...
## modified unetgenerator based on new upsampling technique.
# def unet_generator(output_channels, norm_type='batchnorm'):
#     """Modified U-Net generator model with artifact-free upsampling.
...
#     up_stack = [
#         upsample(512, 3, norm_type, apply_dropout=True),  # (bs, 2, 2, 1024)
#         upsample(512, 3, norm_type, apply_dropout=True),  # (bs, 4, 4, 1024)
#         upsample(512, 3, norm_type, apply_dropout=True),  # (bs, 8, 8, 1024)
#         upsample(512, 3, norm_type),  # (bs, 16, 16, 1024)
#         upsample(256, 3, norm_type),  # (bs, 32, 32, 512)
#         upsample(128, 3, norm_type),  # (bs, 64, 64, 256)
#         upsample(64, 3, norm_type),  # (bs, 128, 128, 128)
#     ]

#     initializer = tf.random_normal_initializer(0., 0.02)
    
#     # Final layer: Uses kernel size 3, stride 1 to avoid artifacts
#     last = tf.keras.layers.Conv2DTranspose(
...
#     skips = reversed(skips[:-1])  # Remove last downsampling layer from skip connections
...
#     # Apply UpSampling2D before the final layer
#     x = tf.keras.layers.UpSampling2D(size=2)(x)  # Final smooth upsampling

#     x = last(x)  # Final transposed convolution
